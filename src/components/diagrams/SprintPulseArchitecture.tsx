...
const SprintPulseArchitecture: React.FC = () => {
  ...
  const topRow = [
    { title: 'Next.js Client', icon: '🖥️', color: '#3B82F6' },
    { title: 'API Gateway', icon: '🔐', color: '#A855F7' },
    { title: 'AI Engine', icon: '🧠', color: '#F59E0B' },
    { title: 'Prisma + PG', icon: '🗄️', color: '#10B981' },
    { title: 'Security', icon: '🛡️', color: '#EF4444' },
  ];

  const bottomRow = [
    { title: 'Workspaces', icon: '👥', color: '#3B82F6' },
    { title: 'Biz Logic', icon: '⚙️', color: '#A855F7' },
    { title: 'Simulation', icon: '📊', color: '#F59E0B' },
    { title: 'External APIs', icon: '🔗', color: '#10B981' },
    { title: 'Auditability', icon: '📋', color: '#EF4444' },
  ];
    ...
    <div ref={ref} className="w-full h-full p-3 sm:p-4 flex flex-col justify-between overflow-hidden">
      ...
      <div className={`mt-auto p-2 bg-slate-800/60 border border-slate-700/40 rounded-lg transition-all duration-700 ${step >= 10 ? 'opacity-100' : 'opacity-0'}`}>
        <p className="text-[10px] text-gray-400">
          <span className="text-yellow-400 font-semibold">Removes 4-6 hrs/week</span> of manual Scrum Master calculations
        </p>
      </div>
    </div>
  );
...
export default SprintPulseArchitecture;
